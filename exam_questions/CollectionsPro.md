...
- ArrayList - быстрая итерация по элементам, тк они расположены в непрерывной области.
    - LinkedList - медленная, из-за доступа по ссылкам между элементами.
...
- ArrayList - меньше дополнительной памяти, кроме массива данных.
    - LinkedList - требует больше памяти для хранения ссылок на соседние элементы.
...
переопределении метода `equals()` всегда переопределяют `hashCode()`. Если метод `equals()` возвращает true,
то `hashCode()`
...
Умножаем его на другое простое нечетное число (популярно 17). Прибавляем хэш-код поля, которое относится к
...
затрудняет их уникальное определение и может снизить эффективность работы структуры данных.

`Хеш-функция`: Когда вы
...
Или можем ответить так, попроще. `Коллизия` - это когда для двух объектов вычисляется одинаковый хеш-код.
