...
Дженерики в Java — это механизм, позволяющий создавать обобщенные классы и методы, которые могут работать с любыми
типами данных, обеспечивая при этом безопасность типов на этапе компиляции. 
Благодаря дженерикам, мы можем использовать одину и ту же структуру данных с разными типами, сохраняя при
этом строгую типизацию и избегая ошибок приведения типов.
...
Это делает программы более безопасными и ясными. 

### Преимущества использования дженериков
+ Безопасность типов на этапе компиляции — меньше ошибок в рантайме.
+ Нет необходимости в явном приведении типов — код становится чище.
+ Универсальность — один класс или метод может работать с различными типами данных.
### Недостатки дженериков
+ Нельзя использовать с примитивами. Тк дженерика работают только с объектами.
+ Стирание типов — информация о типах стирается во время компиляции, и во время выполнения JVM не знает, с каким типом работает дженерик.
...
Информация о Generics в Java доступна на этапе компиляции, но не сохраняется в явном виде в скомпилированном байт-коде. Этот механизм называется Type Erasure (стирание типов).
Type Erasure — это процесс, при котором информация о параметрах типов дженериков удаляется во время компиляции, и компилятор заменяет их на родовые (сырой тип, raw type) или на ограниченный тип, если были установлены ограничения.

Это означает, что во время выполнения JVM не знает, с каким конкретным типом работает параметризованный класс или метод. Все типовые параметры заменяются на Object или на верхнюю границу, если такая установлена.
```java
...
### Где и как хранится информация про Generics?
+ Информация о параметрах типа доступна только на этапе компиляции, чтобы обеспечить безопасность типов и проверку правильности кода.
+ Компилятор проверяет, что вы не добавляете значения неправильного типа, и что операции над параметризованными типами выполняются корректно.
+ При обнаружении ошибок компилятор сообщает о них, что помогает предотвратить потенциальные проблемы в рантайме.
### Стирание типов:
+ После компиляции информация о параметрах типа удаляется из байт-кода.
Например, коллекции List<String> и List<Integer> будут выглядеть одинаково в байт-коде и восприниматься как List.
### Почему используется Type Erasure?
Type Erasure применяется для того, чтобы обеспечить обратную совместимость с кодом, написанным до появления дженериков (до Java 5). Это позволяет использовать новые параметры типов в старом коде, который уже был скомпилирован без поддержки дженериков.
...
Коллизии в Map возникают, когда несколько различных ключей имеют один и тот же хеш-код. В таких ситуациях разные ключи оказываются в одном и том же "кармане" (или "бакете") в хеш-таблице, что приводит к необходимости разрешения этих конфликтов, чтобы обеспечить правильное хранение и доступ к данным.
### Почему возникают коллизии?
Когда вы добавляете ключ в HashMap, Java использует метод hashCode() для вычисления хеш-кода этого ключа. Хеш-код — это число, которое указывает на определённый "карман" в хеш-таблице, где должен храниться этот ключ. Однако:
...
+ Цепочки (Chaining):
Если два ключа попадают в один и тот же "карман", HashMap создаёт связный список в этом кармане.
Новые элементы с тем же хеш-кодом добавляются в этот список.
При доступе к элементам с коллизиями HashMap последовательно проверяет каждый элемент в списке, пока не найдёт нужный ключ.
+ Красно-чёрные деревья (с Java 8):Если количество элементов в одном "кармане" (цепочке) превышает определённый порог (по умолчанию 8), HashMap начинает использовать красно-чёрное дерево вместо связного списка.
Это позволяет улучшить производительность поиска, вставки и удаления с O(n) до O(log n) в случае большого количества коллизий.
...
Для объектов: комбинируйте hashCode() всех полей, используя простые арифметические операции (например, умножение на простое число).
```java
@Override
public int hashCode() {
    int result = 17;
