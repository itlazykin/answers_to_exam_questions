...
| Характеристика         | CountDownLatch	                            | Phaser                                          |
|------------------------|--------------------------------------------|--------------------------------------------------------|
...
CompletableFuture — мощный инструмент в Java для работы с асинхронным программированием. Он предоставляет удобные методы
для управления и комбинирования асинхронных задач.
Основные концепции:

+ Асинхронная задача: CompletableFuture позволяет запускать задачу, которая выполняется в другом потоке.
+ Комбинирование: Легко объединяет несколько задач в цепочки.
+ Обработка результатов: Удобные методы для обработки успешного завершения или ошибок.

| Метод         | Описание	                                                  |
|---------------|------------------------------------------------------------|
| supplyAsync   | Запускает задачу, которая возвращает результат             |
| runAsync      | Запускает задачу без результата                            |
| thenApply     | Преобразует результат                                      |
| thenAccept    | Обрабатывает результат (без преобразования)                |
| thenCombine   | Комбинирует два CompletableFuture                          |
| allOf, anyOf  | Ожидает выполнения всех или хотя бы одной задачи           |
| exceptionally | Обрабатывает исключение и возвращает значение по умолчанию |
| handle        | Обрабатывает результат и исключение одновременно           |
