...
- forEach() - используется для выполнения действия над каждым элементом потока. Он применяется,
...
- forEach() - это конечная операция. Выполняет действие для каждого элемента потока, завершая поток. Он применяется,
  когда вам нужно выполнить какую-то операцию над каждым элементом потока, но при этом не требуется возвращать
  результат.
...
Var - это ограниченный идентификатор, который упрощает создание переменных. Стоит подчеркнуть важную особенность - var
не является ключевым словом согласно спецификации языка. Соответственно var можно использовать в качестве имени
переменной. Var может использоваться только в сочетании с данными, var можно использовать в циклах.
