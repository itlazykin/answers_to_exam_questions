...
Если данные изменились, транзакция выбрасывает исключение `OptimisticLockException`.
...
+ Вместо ленивой загрузки можно настроить жадную загрузку через аннотацию @OneToMany(fetch = FetchType.EAGER) или
