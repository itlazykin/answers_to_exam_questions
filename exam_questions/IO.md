...
- next() — считывает следующее слово до первого пробела или разделителя.
- nextLine() — считывает всю строку целиком.
- nextInt(), nextDouble(), nextBoolean() и другие — считывают значение определенного типа.
- hasNext(), hasNextInt(), hasNextDouble() и так далее — проверяют, есть ли в потоке данные нужного типа.
