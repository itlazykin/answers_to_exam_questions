...
Java IO — это классическая модель ввода-вывода, основанная на потоках (streams). Используется для работы с файлами,
сетью и другими источниками данных. Используется потокоориентированный подход.
Данный подход организует чтение из потока или запись в поток последовательно по несколько байт в один момент времени. 
Операции чтения/записи блокируют поток выполнения до завершения.
...
Улучшенная версия IO, использует каналы и буферы для асинхронного и неблокирующего ввода/вывода, что повышает производительность при работе с большими объемами данных.

Буфер — это область памяти, где данные временно хранятся.

Каналы — это объекты, через которые данные читаются из источников или записываются в них.

NIO позволяет работать более эффективно, поскольку данные можно читать или записывать асинхронно, без блокировки программы.
...
IO, NIO и NIO.2 - это не замены друг другу. Скорее они дополняют друг друга для предоставления более широких
возможностей работы системы ввода-вывода Java.

[К оглавлению](#IO)

# 5. Что такое Scanner?

Scanner — это класс для считывания ввода (источник данных может быть любым - клавиатура, файл), и он использует токены для разделения потока
данных на части, которые потом можно обработать. Эти токены затем могут быть преобразованы в различные типы данных,
такие как
числа, строки или другие примитивные типы. Токены — это как маленькие кусочки данных, которые позволяют удобно работать
с большими строками или вводом(например у строки `"Я учу джаву"` - 3 токена: я, учу, джаву
По умолчанию Scanner использует пробел в качестве
разделителей токенов. Но ты можешь задать свой разделитель, используя метод useDelimiter().

[К оглавлению](#IO)

# 6. Как работает Scanner внутри?

- Scanner использует буфер для чтения данных из источника (например, строки, файла), чтобы ускорить обработку, не обращаясь каждый раз к источнику.
- Он делит входные данные на части (токены) по разделителям (пробелы, символы новой строки и т.д.), используя регулярные выражения.
- Метод hasNext(): Проверяет, есть ли еще токены для чтения. Возвращает true, если токены остались, и false, если все данные прочитаны.
- Методы чтения данных: next() извлекает токен и возвращает его. Для чисел есть nextInt(), для строк — nextLine() и так далее.
- Если данные не соответствуют ожидаемому типу (например, пытаешься прочитать строку как число), выбрасывается исключение (InputMismatchException).
- Через useDelimiter() можно задать собственные правила для разделения токенов, используя регулярные выражения.

Работа сканера - похожа на работу итератора, но с разницей в типах данных, с которыми работает каждый из них. Итератор работает с коллекциями, сканер с потоками данных.
Причем большинство методов можно разделить на hasNextTYPE и nextTYPE, где TYPE - это тип по шаблону которого будет происходить отделение токенов друг от друга. Например, hasNextInt(), nextInt().
...
Байтовый поток используется для работы с бинарными данными (например, изображения, аудио, файлы). Он читает и записывает данные побайтово, без преобразования в символы.
...
Основные классы-реализации InputStream (потоки ввода):
    ...
    - FilterInputStream - реализует абстрактный класс InputStream. От него наследуются классы:
...
Основные классы-реализации OutputStream (потоки вывода):
    ...
    - FilterOutputStream - реализует абстрактный класс OutputStream. От него наследуются следующие классы:
...
Символьный поток используется для работы с текстовыми данными. Он автоматически преобразует символы в байты и наоборот с использованием кодировки (например, UTF-8).
...
Буферизированный поток — это поток, который использует внутренний буфер для хранения данных, чтобы уменьшить количество
операций с реальным источником (например, файлом), что повышает производительность.

Принцип работы:

    Данные сначала записываются в буфер, а затем, когда буфер заполняется, записываются в реальный источник.
    Аналогично для чтения — данные сначала считываются в буфер и затем извлекаются по мере необходимости.

Символьные - BufferedReader, BufferedWriter, байтовые - BufferedInputStream, BufferedOutputStream.
...
Это происходит потому что JVM запущена не из стандартной интерактивной консоли операционной системы, а из среды разработки, поэтому у неё нет
связи с консолью (командной строкой), соответственно, мы не можем получить объект консоли и взаимодействовать с ней.
Чтобы получить доступ к консоли (командной строке), нужно запустить программу непосредственно из неё.

Чувствительные данные нужно хранить в массиве байтов. Если хранить данные в обычной строке, то это чревато тем, что
вредоносные программы могут сохранить дамп памяти и изъять оттуда все логины, пароли и любую другую информацию, которая
находилась в памяти на момент сохранения дампа. Дамп памяти - это текущее содержимое оперативной памяти одного процесса
или всей ОС.
При работе с чувствительными данными важно, чтобы после получения их в память время нахождения этих данных в памяти было
минимальным. Поэтому нужно хранить данные в массиве байтов, содержимое которого мы можем сразу
же "затереть" после получения и обработки чувствительных данных с помощью метода fill.
Чувствительные данные — это данные, которые требуют особой защиты, так как их утечка может привести к нежелательным последствиям, например, к финансовым потерям или компрометации безопасности.
...
данных,поскольку char[] можно быстро обнулить после использования .
```
...
Потоки объектов представлены в пакете java.io классами ObjectInputStream и ObjectOutputStream. ObjectOutputStream
превращает объект в его байтовое представление.
ObjectInputStream восстанавливает объект из байтового представления в обычное. Эти процессы называются сериализацией и
десериализацией соответственно.
С помощью этих операций можно сохранить объект (состояние объекта) в виде последовательности байт, удобной для передачи
по сети, и восстановить эту последовательность байт обратно в объект, например, на другом компьютере.
...
Path — это объект, представляющий путь к файлу или директории в файловой системе.
...
Список файлов можно получить через File.listFiles(), сразу загружает весь массив File[] в память.

```java
public static void main(String[] args) {
    File dir = new File("C:/example"); // Путь к директории
...
Files.list(), возвращает Stream<Path>, который читает файлы постепенно благодаря ленивой загрузке, что экономит память. 
...
До JDK 7 не было готовой возможности переместить файл в другую директорию, но есть 2 варианта как это можно сделать
вручную:

- Если переместить нужно содержимое файла, то достаточно применить метод File.renameTo(), то есть просто переименовать
  его, тем самым достигая результата "перемещения" данных в файл с заданным именем.

- Если требуется переместить файл в другую директорию, то нужно скопировать содержимое файла в новый файл в другой
  директории, после чего старый файл удалить.
...
1. Для чтения базовых атрибутов (дата создания, последняя модификация и т.д.) можно использовать интерфейс
...
2. Для изменения атрибутов, таких как время последней модификации, используется метод Files.setAttribute().
...
3. Для управления правами доступа в Java существует интерфейс PosixFileAttributes и класс PosixFilePermissions. Они
...
4. Java позволяет работать с пользовательскими атрибутами, такими как метки или заметки, которые можно добавлять к
...
1. Старый, но простой способ создать файл — это использовать класс File. Метод createNewFile() позволяет создать новый
...
2. Современный и более предпочтительный способ создания файла — это использование класса Files вместе с интерфейсом
...
3. Если нужно создать временный файл, который не должен сохраняться после завершения
...
4. Если нужно создать файл с определёнными правами доступа, это можно сделать с помощью опций FileAttribute в
   Files.createFile().
...
1. Старый, создания директории — это метод mkdir() или mkdirs() класса File.
...
2. Современный и рекомендуемый способ создания директории — это использование класса Files вместе с интерфейсом Path.
...
Для чтения данных из файла можно использовать старые способы с FileReader и BufferedReader, а также более современные
подходы с Files.readAllLines() или Files.newBufferedReader().
...
Главное их отличие только в том, что PrintStream читает побайтово, а PrintWriter посимвольно.
...
Класс RandomAccessFile позволяет работать с файлами произвольно, что дает возможность читать и записывать данные в любой части файла, а не только последовательно. Это полезно, например, для работы с большими файлами или базами данных

Класс RandomAccessFile не наследуется от классов InputStream и OutputStream.
...
- seek(номер позиции) - перемещает указатель на указанную позицию.
...
RandomAccessFile - это последовательность байтов, поэтому этот и предыдущий методы
...
Если нужно работать только с символьными данными, то рекомендуется использовать символьные потоки и их
...
Сокет — это точка взаимодействия между двумя программами в сети, которая позволяет отправлять и получать данные. В Java
для работы с сокетами используются классы Socket и ServerSocket, которые обеспечивают клиент-серверное взаимодействие
через протоколы TCP или UDP
...
JSON (JavaScript Object Notation) — это легкий формат обмена данными, который используется для хранения и передачи
структурированных данных в текстовом виде. Он легко читается человеком и парсится программами, что делает его популярным
...
Для преобразования POJO в JSON и обратно можно использовать библиотеки Jackson или Gson. В Jackson это делается с
помощью класса ObjectMapper, который имеет методы writeValueAsString для преобразования в JSON и readValue для
преобразования обратно в объект. В Gson аналогичные методы предоставляются через toJson и fromJson.

+ Для использования Jackson в проекте, необходимо добавить зависимость в файл pom.xml (для Maven) или build.gradle (для Gradle).
...
Для преобразования POJO в XML и обратно можно использовать JAXB или Jackson. В JAXB мы добавляем аннотации к POJO
классу, создаём JAXBContext и используем Marshaller и Unmarshaller для преобразования.
...
- Поле serialVersionUID - уникальный идентификатор версии сериализованного класса, необходим для обеспечения механизмов версионности,
  т.е. нужен JVM для понимания, что сериализованный объект при десериализации имеет те же поля класса, методы и пр.
